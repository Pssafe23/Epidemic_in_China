            ...
            option = {
                ...
                yAxis: [{
                    ...
                    axisLabel: {  // 坐标轴的标签，也就是那些数字
                        ...
                        formatter: function(val) {  // 这val就是输出的数字
                            if (val > 10000) {
...
(function () {
    $.ajax({
        ...
        success: function (data) {
            ...
            var d1_name = '累计死亡'
            var d2_name = '累计确诊'
            var dead = data.dead;  // 死亡人数
            var confirm = data.confirm;  // 确诊人数
            var rate = data.dead_rate;  // 死亡率
            var option;
            option = {
                title: {
                    text: rate.toString() + "%",
...
(function () {
    $.ajax({
        ...
        success: function(trafficWay) {
            ...
            var data = [];
            var color=['#00ffff','#00cfff','#006ced','#ffe000','#ffa800','#ff5b00','#ff3000','#8A2BE2','#A52A2A']
...
(function() {
    $.ajax({
        ...
        success: function (json_data) {
            ...
            var years = json_data["year_month"];  // 2022-09
            // console.log(years.length)
            var province = json_data["province"];
            var playlist_data = json_data["confirm_add"]  // 相当于统计各省每个月的新增人数
